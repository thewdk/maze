...
void	renderWallProjection(void)
{
	...
	for (int x = 0; x < NUM_RAYS; x++)
	{
		...
		textureWidth = upng_get_width(SDL_textures[texNum]);
		textureHeight = upng_get_height(SDL_textures[texNum]);
		...
		for (int y = wallTopY; y < wallBottomY; y++)
		{
			...
			wallTextureBuffer = (color_t *)upng_get_buffer(SDL_textures[texNum]);
