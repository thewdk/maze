...
void	renderSpriteProjection(void)
{
	...
	for (int i = 0; i < numVisibleSprites; i++)
	{
		...
		textureWidth = upng_get_width(SDL_textures[sprite.texture]);
		textureHeight = upng_get_height(SDL_textures[sprite.texture]);
		...
		for (int x = spriteLeftX; x < spriteRightX; x++)
		{
			...
			for (int y = spriteTopY; y < spriteBottomY; y++)
			{
				if (x > 0 && x < WINDOW_WIDTH && y > 0 && y < WINDOW_HEIGHT)
				{
					...
					spriteTextureBuffer = (color_t *)upng_get_buffer(SDL_textures[sprite.texture]);
